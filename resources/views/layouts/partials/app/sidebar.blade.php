<div x-data="{ sidebarOpen: true }" class="flex h-screen">
    ...
    <aside id="logo-sidebar"
        ...
        <!-- Header del sidebar -->
        <div class="flex items-center justify-between px-3 border-b border-black dark:border-white">
        ...
        <div class="flex-1 px-3 pt-6 pb-4 overflow-y-auto">
            <div class="mb-4">
                <x-label for="employee_number" value="{{ __('N° de Nómina') }}" />
                <x-input id="employee_number" type="text" class="block w-full mt-1 sm:text-sm"
                    value="{{ Auth::user()->employee_number }}" readonly />
            </div>

            <div class="mb-4">
                <x-label for="name" value="{{ __('Nombre') }}" />
                <x-input id="name" type="text" class="block w-full mt-1 sm:text-sm"
                    value="{{ Auth::user()->name }}" readonly />
            </div>

            <div class="mb-4">
                <x-label for="last_name" value="{{ __('Apellidos') }}" />
                <x-input id="last_name" type="text" class="block w-full mt-1 sm:text-sm"
                    value="{{ Auth::user()->last_name }}" readonly />
            </div>

            <div class="mb-4">
                <x-label for="group" value="{{ __('Grupo') }}" />
                <x-input id="group" type="text" class="block w-full mt-1 sm:text-sm"
                    value="{{ Auth::user()->group ?? 'Sin grupo' }}" readonly />
            </div>

            <div class="mb-4">
                <x-label for="area" value="{{ __('Área') }}" />
                <x-input id="area" type="text" class="block w-full mt-1 sm:text-sm"
                    value="{{ Auth::user()->area?->name }}" readonly />
            </div>

            <div class="mb-4">
                <x-label for="supervisor_name" value="{{ __('Supervisor (Jefe directo)') }}" />
                <x-input id="supervisor_name" type="text" class="block w-full mt-1 sm:text-sm"
